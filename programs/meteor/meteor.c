...
void calc_six_rotations(char piece, char index)
{
    ...
    for (rotation = 0; rotation < 6; rotation++) {
        if (piece != 3 || rotation < 3) {
            ...
            if (cells_fit_on_board(cell, piece)
                && !has_island(cell, piece)) {
...
int triple_is_okay(char row1, char row2, char row3, int even)
{
    if (even) {
         ...
         * row3: 011??  00110  ?????  ?????
         */
        return ((row1 == 0x03) && (row2 == 0x0B)
                && ((row3 & 0x1C) == 0x0C)) || ((row1 == 0x01)
                                                && (row2 == 0x05)
                                                && (row3 == 0x06))
            || ((row1 == 0x19) && (row2 == 0x11)) || ((row1 == 0x15)
                                                      && (row2 == 0x11));
    } else {
        /* There are two cases:
         * row1: 10011  10101
        ...
        for (row2 = 0; row2 < 32; row2++) {
            for (row3 = 0; row3 < 32; row3++) {
                ...
                if (result1 == FALSE && result2 == TRUE
                    ...
                    bad_even_triple[row1 + (row2 * 32) + (row3 * 1024)] =
                        FALSE;
                else
                    bad_even_triple[row1 + (row2 * 32) + (row3 * 1024)] =
                        result1 || result2;
                ...
                if (result1 == FALSE && result2 == TRUE
                    ...
                    bad_odd_triple[row1 + (row2 * 32) + (row3 * 1024)] =
                        FALSE;
                else
                    bad_odd_triple[row1 + (row2 * 32) + (row3 * 1024)] =
                        result1 || result2;
    ...
    for (sol_no = 0; sol_no < 10; sol_no++) {
        ...
        for (index = 0; index < 50; index++) {
            if (sol_mask & 1ULL) {
                ...
                solutions[solution_count + 1][49 - index] =
                    sol_nums[sol_no];
            }
            sol_mask = sol_mask >> 1;
        }
...
int main(int argc, char **argv)
{
    ...
    qsort(solutions, solution_count, 50 * sizeof(signed char),
          solution_sort);
    pretty(solutions[0]);
