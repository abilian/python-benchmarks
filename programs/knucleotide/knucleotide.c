...
struct tp *tp_create(int max_jobs)
{
...
void tp_destroy(struct tp *pool)
{
    free(pool->jobs);
...
void tp_add_job(struct tp *pool, void *job, void *param)
{
...
void tp_run(struct tp *pool, int max_threads)
{
...
char *read_stdin(int *stdin_size)
{
    struct stat stat;
    fstat(0, &stat);
...
uint64_t pack_key(char *key, int len)
{
    uint64_t code = 0;
    for (int i = 0; i < len; i++) {
...
void print_freqs(struct print_freqs_param *param)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        output_pos +=
            snprintf(output + output_pos, output_size - output_pos,
                     "%s %.3f\n", buffer,
                     counts[i].val * 100.0f / total_count);
    }
